...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_TRUNK)
	{
	    if(response)
	    {
	        ...
	        if(listitem == 2)
	        {
	            ShowPlayerDialog(playerid, DIALOG_SLOT1, DIALOG_STYLE_LIST, "Storage Slot 2", "Take Weapon\nStore Weapon", "Select", "Cancel");
			}
	        if(listitem == 0)
	        {
	            ShowPlayerDialog(playerid, DIALOG_SLOT1, DIALOG_STYLE_LIST, "Storage Slot 3", "Take Weapon\nStore Weapon", "Select", "Cancel");
			}
		}
	}
